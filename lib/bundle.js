...
class Game {
  ...
  spawnObstacles() {
    ...
    if (Math.random() > 0.50 && this.obstacles.length < 3) {
  ...
  updateObstacles() {
    for (let i = 0; i < this.obstacles.length; i++) {
      ...
      if (this.obstacles[i].isOutOfBounds()) {
        this.obstacles.splice(i,1);
      }

      if(this.runner.isCollidedWith(this.obstacles[i])) {
        // this.stop();
        debugger
      }
    }
...
class Runner {
  ...
  isCollidedWith(obstacle) {
    ...
    if (obstacleVec.imgW === 0 || obstacleVec.imgH === 0) { return false; }

    let vec1 = {}, vec2 = {}, vec3 = {};
    vec1.x = (this.runningHomer.imgW / this.runningHomer.numberOfFrames) / 2;
    vec1.y = this.runningHomer.imgH / 2;
    vec2.x = this.jumpingHomer.xPos;
    vec2.y = this.jumpingHomer.yPos;
    vec3.x = obstacleVec.xPos;
    vec3.y = obstacleVec.yPos / 2;
    if (vec1.x >= vec3.x && !this.jumping) {
      return true;
    }
  }
...
class Obstacle {
  ...
  vectors() {
    return {
      xPos: this.obstacle.scroll,
      yPos: this.obstacle.yPos,
      imgW: this.obstacle.imgW / this.frames,
      imgH: this.obstacle.imgH
    };
  }
}
