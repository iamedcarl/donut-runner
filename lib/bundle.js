...
class Sprite {
  ...
  renderObstacle() {
    ...
    let sh = this.imgH;
    let dx1 = this.scroll;
    let dx2 = this.scroll + CANVAS_X_SIZE;
    let dy = this.yPos;
    let dw = (this.imgW / this.numberOfFrames) * this.scale;
    let dh = this.imgH * this.scale;

    this.ctx.drawImage(img, sx, sy, sw, sh, dx1, dy, dw, dh);
    this.ctx.drawImage(img, sx, sy, sw, sh, dx2, dy, dw, dh);
...
const IMGS = [
  ...
  ['https://s3.us-east-2.amazonaws.com/donut-runner/copcar.png', 1]
];
...
class Game {
  ...
  createObstacles(ctx, speed) {
    ...
    this.obstacleCar = new __WEBPACK_IMPORTED_MODULE_2__obstacle__["a" /* default */](ctx, IMGS[5][0], IMGS[5][1], speed);
  }
